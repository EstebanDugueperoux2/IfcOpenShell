...
class IfcopenshellConan(ConanFile):
    ...
    options = {
        ...
        "with_cityjson_support": [True, False],
    ...
    exports_sources = "aws/**", "choco/**", "CMakeLists.txt", "utilities.cmake", "src/**", "docs/**", "**.md"
    ...
    def requirements(self):
        self.requires("boost/1.88.0", transitive_headers=True, transitive_libs=True, force=True)
        ...
        if self.options.build_ifcgeom and self.options.with_cgal or self.options.ifcxml_support:
            self.requires("libxml2/[^2.12.5]")

        if self.options.with_cityjson_support:
    ...
    def generate(self):
        ...
        tc.cache_variables["CITYJSON_SUPPORT"] = self.options.with_cityjson_support
        
        # tc.variables["JSON_INCLUDE_DIR"] = self.dependencies['nlohmann_json'].cpp_info.includedirs[0]
